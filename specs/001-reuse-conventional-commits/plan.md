...
| VI. Verification By Real Invocation | **This is the feature** | This is the repository's first real invocation of one of its own reusable workflows, at a ref that resolves to the code under review — VI's first sentence, whose wording #20 settled. `conventional-commits.yml` reads nothing from caller-side configuration, so the second does not bite. It is itself verified the same way: the deliberate-break scenario in [quickstart.md](./quickstart.md) is the acceptance test, and lint alone does not close it. |
