...
| **`GITHUB_TOKEN` needs a repository setting that is currently off**, and probably one *Approve and run* click per release | `Allow GitHub Actions to create and approve pull requests` is off in this repository and in `github-actions-test` — probed, not assumed (research.md decision 10): `POST /pulls` answers `GitHub Actions is not permitted to create or approve pull requests` (403). Beyond that, the `main` ruleset requires three checks, which a bot-opened pull request is documented to create in an approval-required state. | A GitHub App token via `actions/create-github-app-token` needs **neither** the setting nor the click, at the cost of two secrets, an app to register and one more pinned action. A PAT removes the click by introducing a long-lived human-owned credential into a release path built so no credential reaches a workspace. `GITHUB_TOKEN` stays the choice on record — the click is *before* approval, so FR-009's "no separate manual trigger afterwards" still holds — but the setting makes this worth re-taking rather than inheriting. Enabling it is the owner's call, not a planning step. |
| **The click itself is still unverified** | Whether the required checks appear gated or do not appear at all cannot be observed while the setting is off, because the pull request never exists. | Nothing cheaper reaches it: the probe already went as far as the setting allows. It resolves the moment the setting is turned on, or becomes moot if the App token is adopted. |
