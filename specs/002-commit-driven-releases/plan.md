...
the release fires once `ci / CI` reports green on the merge commit. The proposal is opened by a GitHub
App installation token, because `GITHUB_TOKEN` cannot open a pull request here without granting Actions
...
| **VI. Verification By Real Invocation** | **Partial — see Complexity Tracking** | `push: main` and `workflow_run` cannot be exercised from a feature branch. `release.yml` gains a `dry-run` dispatch that runs the real code; `release-proposal.yml` cannot be dispatched at all before merge — a new workflow is invisible to `workflow_dispatch` — so it is exercised by a temporary `push` trigger on this branch. The first release after merge exercises the automatic triggers. |
