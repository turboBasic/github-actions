...
- [x] T016 Run quickstart rung 3: dispatch `release.yml` with `dry-run: true` from this branch, and read the `Read the consumer-surface filter` and `Render the notes` step logs. This proves our path is unchanged, including that `read -ra` handles the flag line — the one shell behaviour no offline test covers. **Done, and it found that this rung cannot exercise `decisions.py` at all**: `release.yml` reaches the action through `@v4`, so `${GITHUB_ACTION_PATH}` is the tagged download. Run 34059521759 logged the constants' filter, not the table's. quickstart rung 3 corrected, and T016a added.
- [x] T016a Run quickstart rung 3a: on the merge to `main`, watch `release-proposal.yml`'s `Decide the version` step and confirm the surface flags come from `[tool.turbobasic-release]`. It calls `decisions.py` by in-repo path, so it is the only thing that exercises the module under review before `@v4` moves.
- [x] T017 Cut the release: merge, let `release-proposal.yml` open its proposal, and check the increment is a **minor** rather than accepting the number — a patch proposal would mean the surface filter is not seeing this change, which is itself a finding ([research.md D7](./research.md#d7)).
- [x] T018 [US3] Run quickstart rung 4 in `github-actions-test` **before** it declares a surface: dispatch with `dry-run: true`, confirm the notice and an unfiltered refusal. This state disappears the moment T019 lands, so it is seen now or not at all (SC-003).
- [x] T019 [US1] [US2] Run quickstart rung 5 in `github-actions-test`: add `[tool.turbobasic-release]` declaring `src/**`, then two dry-run dispatches — a breaking commit in `src/` under a non-major must be refused, and a breaking commit touching only its own workflows under a patch must proceed. Both directions, because either alone proves half of SC-002.
- [x] T020 Run quickstart rung 6: a real release in `github-actions-test` with `dry-run` dropped. Confirm the tag, the release, the moved major tag, and that the notes describe the **whole** range rather than the filtered one (FR-011).
