...
- [ ] T016a Run quickstart rung 3a: on the merge to `main`, watch `release-proposal.yml`'s `Decide the version` step and confirm the surface flags come from `[tool.turbobasic-release]`. It calls `decisions.py` by in-repo path, so it is the only thing that exercises the module under review before `@v4` moves.
