...
| **VI. Verification By Real Invocation** | PASS, with a rung that cannot be skipped | Behaviour turns entirely on the released repository's own file, so the offline suite and a dry run here prove only our path. `github-actions-test` has to exercise both directions and the undeclared case at the ref it pins. Ladder in [quickstart.md](./quickstart.md); the finding that makes it cheap is that **a dry-run dispatch reaches the refusal without cutting a tag**, because `check-notes` runs whenever `verify.outputs.proceed` is true and a dry run makes it true. |
