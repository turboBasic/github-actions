...
- [X] T043 Write a rule into `CLAUDE.md` three ways — a bare paragraph, a paragraph under a new heading, and a gloss column in the layer 3 table — and confirm each fails the layer 4 check, then revert. **Added in review, and it found the check inert.** The split ran on whitespace-collapsed text, so `CLAUDE.md` reduced to six sentences each beginning `#` or `|`, and none of the three probes failed. Fixed by splitting the raw text and stripping markdown structure; all three fail now.
- [X] T044 Anchor the layering rule and the layer 1 admission bar, and confirm both new rows fail on a restatement. **Added in review, and it found both facts already copied** into `docs/instruction-layers.md` — the write-up restated the direction rule verbatim and paraphrased the admission bar one word off. Both now sit there as blockquotes attributed to their owners, and the check skips a line inside a blockquote: rewording until the phrase differs would clear the check while leaving the second copy, so the exemption is what keeps the gate from becoming a dial. Watched fail by restating the admission bar in `CONTRIBUTING.md`, and again when a mistaken `git checkout` restored the unquoted copy.
- [X] T045 Move the map to `AGENTS.md` so one copy serves both tools, and confirm the two new checks fail. **Added in review.** Layer 2 defers to the entry point six times — `:13`, `:33`, `:108`, `:146`, `:175`, `:236` — and Copilot's entry point answered none of them, because the map was in `CLAUDE.md`. `AGENTS.md` is auto-detected by Copilot and imported by `CLAUDE.md`, which is the arrangement the Claude Code memory docs prescribe. `chat.useAgentsMdFile` is set in `.vscode/settings.json`. Verified against the published docs rather than assumed: Copilot ranks agent instructions below repository-wide instructions, which is harmless here because navigation states no rule.
...
- **Polish (Phase 7)**: depends on Phase 5 for T035–T038 and T043–T045, and on everything for T042.
